...
def main(mode):
    ...
    model.to(device)
    model.load_state_dict(torch.load('./checkpoints_re/CP_epoch5.pth'))  #load trained model state***************

    dir_img = '/media/jihu/data/dataset/AAA/AAAGilDatasetPos/raw/'
    dir_mask = '/media/jihu/data/dataset/AAA/AAAGilDatasetPos/mask/'
    ...
    if 'test' in mode:
        ...
        in_files = dir_img + '35139583_20200821_0119.png'
        mask_name = dir_mask + '35139583_20200821_0119.png'
        ...
        prediction = predict_img(net=model, full_img=img,
                           scale_factor=1, out_threshold=0.5, device=device)
        ...
        img_overlap = cv2.addWeighted(img_mask_gt, 1, img_mask, 1, 0)
    ...
    if 'eval' in mode:
        dataset_test = torch.load('./dataset/dataset_test.pth')
        ...
        for i in range(num_test):
            img_name = dir_img + dataset_test[i]['idx'] + '.png'
            mask_name = dir_mask + dataset_test[i]['idx'] + '.png'
